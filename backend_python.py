...
import sys, subprocess, time, os, re, numpy, requests, random, shutil, matplotlib, pandas
...
#    print(prot_ids)
   ...
   # if os.path.exists(fetched_file_name):
       # os.remove(fetched_file_name)
    #else:    
...
with open(aligned_sequences, 'w') as AS:
    subprocess.run(f"clustalo -i {fetched_file_name} -o {aligned_sequences} --force",shell=True)
print(aligned_sequences)

#2.f Message to the user
print("The basic plan of this site is to perform conservation analysis, and the results are as follows:")
